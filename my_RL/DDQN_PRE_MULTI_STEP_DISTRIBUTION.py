...
from distributional_NN import tensorflowMLP
...
class myDQN:
    def __init__(self,
        ...
        self.neuralnet = tensorflowMLP(self.state_n, 20, out=self.env.action_space.n,batch=batch)
        self.point = 0
        self.Q_list = []
        self.batch = batch
        self.IsRender = False
        self.test = 0
        self.multi_step = 3#when multy_step=0 it become DDQN_PER
    ...
    def learn(self):
        ...
        while iteration < self.maxIteration:
            reward_ = 0
            self.epsilon -= 1.0/(self.maxIteration)
